...
In some cases, you might use fallback values for missing data, especially if the missing portion is not critical. The choice of fallback values depends on the context (e.g., using 0, averages, or historical data).
...
Ensure each record has a unique identifier. When loading data, check if the record's unique identifier already exists in the target data store. If it does, you can choose to skip or update the existing record.

### Primary Key Constraints

If your target data store supports primary key or unique constraints (like a relational database), use them to prevent duplicates at the database level.
...
Implement a system of watermarking to process only new or updated records since the last successful ETL run. Use a timestamp or incrementing column in your source data to filter records.

### Change Data Capture (CDC)

Leverage CDC techniques if your source system supports it. CDC allows you to capture only changes (inserts, updates, deletions) since the last extraction.

## State Management

### Checkpoints 

Maintain checkpoints or logs of processed data. Before processing, check if the data has already been processed by comparing it against your checkpoints.

### Idempotence

Ensure that your processing logic is idempotent. This means processing the same data multiple times does not change the outcome after the first successful processing.
...
Design your ETL to handle late-arriving data gracefully. This might involve reprocessing data for a certain period (lookback window) or upserting records based on their unique identifiers.

## Data Quality Checks

### Post-Processing Checks

Implement data quality checks after your ETL process. This can help identify and rectify duplicates that slip through.

## Leveraging Database Features

### Upserts/Merge

Use database features like upserts (insert or update) or merge operations to handle duplicates directly at the database level, if your target storage supports it.



# 3 How would you prevent breaking production when making changes to existing pipelines?

Here are several approaches you can take to prevent breaking production when implementing changes to ETL pipelines with Spark and Scala.

## Version Control and Code Reviews

### Use Version Control

Ensure all pipeline code is under version control using tools like Git. This allows you to track changes, revert to previous versions if needed, and understand the history of modifications.
...
Write unit tests for your Scala code to test individual components in isolation, ensuring they perform as expected.

### Integration Testing

Test how different components of your pipeline interact with each other and with external systems.

### End-to-End Testing

Run your ETL pipeline from start to finish in a controlled environment to ensure it works as expected with real or realistic data.

## Environment Isolation

### Development, Staging, and Production Environments: 

Use separate environments for development, testing, and production. This allows you to test changes in non-production environments that mimic production as closely as possible.
...
Use Spark's monitoring capabilities to keep an eye on your ETL jobs' performance and health. Monitor key metrics and logs to detect issues early.

### Set Up Alerting: 

Configure alerts for anomalies or failures in your ETL processes, so you can quickly respond to issues before they affect production.
